...
/*
  ...
  */
  ...
  pixels.begin();
  pixels.show();
  Sen1 = createWriter( "sen1.txt" );
  Sen2 = createWriter( "sen2.txt" );
  Sen3 = createWriter( "sen3.txt" );
  Sen4 = createWriter( "sen4.txt" );
  Sen5 = createWriter( "sen5.txt" );
  Serial.begin(9600);
}
...
/*
  ...
  for(int i = 0; i < 6; i ++){
    ...
    if (mySerial.available() > 0 ) {
        String value = senVal;
        //Writing it in these if statements due to the fact that you cant prase strings in arduino
        if ( value != null ) {
            if ( i == 1 ) {
                Sen1.println( value );
            }
